...
2. you could create a mean of the scaled sum scores. this would allow both type of stressors to be equally weighted. This is the approach we have used in MARP and LORA which is described in the [FRESHMO paper](). 
...
Consequently, in longitudinal studies we are often interested in longer time periods rather than single time points. For example in the longitudinal samples MARP and LORA we look at period of 9 months to ca. 3.5 years [see Petri-Romão et al.](). 
...
## 2 TODO FROM HERE Model for normative stressor reactivity
...
In studies where you compare different time points, for example intervention studies, we have chosen multilevel modelling (see [Petri-Romão et al. (2024)]())
...
EP_multi <- ds %>% 
  ...
  lme4::lmer(scale(P)~scale(E)+ (1|subject.ID),data=.) %>% 
  summary()
...
As specified in the FRESHMO paper [Kalisch et al.](), the relationship between E and P is mostly linear, but could also be quadratic. 
...
### 3.1 over entire time period

#### 3.1.1 extract intercept and slope
```{r}
intercept  <- EP_norm$coefficients[1]
E_slope  <- EP_norm$coefficients[2]
```
#### 3.1.2 predicted P

```{r}
 ds$P_t1.t6_pred<- intercept  + scale(ds$E_t1.t6)*E_slope  
```
#### 3.1.3 SR score
```{r}
 ds$sr_t1.t6<- scale(scale(ds$P_t1.t6) - ds$P_t1.t6_pred)
```
### 3.2 single time points
...
#### 3.2.3 SR score
```{r}
 ds$sr_single<- scale(scale(ds$P) - ds$P_pred_multi)
```

### extra - based on quadratic model
```{r}
intercept_quad  <- EP_quad$coefficients[1]
E_slope_b  <- EP_quad$coefficients[2]
E_slope_a  <- EP_quad$coefficients[3]

```
```{r}
 ds$P_t1.t6_pred_quad <- intercept_quad  + scale(ds$E_t1.t6)*E_slope_b  + (scale(ds$E_t1.t6)^2)*E_slope_a  
```

```{r}
 ds$sr_t1.t6_quad<- scale(scale(ds$P_t1.t6) - ds$P_t1.t6_pred_quad)
```
